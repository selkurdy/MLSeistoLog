...
python mlseistocatlog.py allsegy.txt AllFACIES_GROUP.csv --startendslice 1000 1010
python mlseistocatlog.py allsegy.txt AllFACIES_GROUP.csv --startendslice 1000 1010
...
def  getcommandline():
    ...
    parser.add_argument('--cbciterations',type=int,default=500,help='Learning Iterations, default =500')
    parser.add_argument('--cbclearningrate',type=float,default=0.03,help='learning_rate. default=0.03')
    parser.add_argument('--cbcdepth',type=int,default=2,help='depth of trees. default=2')
    ...
    parser.add_argument('--balancetype',choices=['ros','smote','adasyn'],default=None,
        help='Random Oversampling,Synthetic Minority Oversampling,Adaptive Synthetic Sampling,default=None')
    parser.add_argument('--nneighbors',type=int,default=1,help='use only for smote and adasyn')
    parser.add_argument('--outproba',action='store_true',default=False,
        help='Output probabilities. default is output class')
    parser.add_argument('--classproba',type=int,default=0,help='output probabilities of class #.default=0')
...
def main():
    ...
    if cmdl.wellscsv:
        ...
        classnames = allwells[wcols[-1]].unique().tolist()
        # cols before I add pred col
        print(f'class names: {classnames}')
        # classnames.remove('nan')
        del classnames[0]
        del classnames[-1]
        # remove last class because it was added in preparecatlogs.py

        print(f'class names after removal: {classnames}')

        lognamepred = logname + 'pred'
        wcols.append(lognamepred)

        probacol = classnames.index(cmdl.classproba)
        #  get index of requested class
    ...
    if cmdl.segyfileslist:
        ...
        if cmdl.outdir:
            if cmdl.outproba:
                outfpsegy = os.path.join(cmdl.outdir,wfname) +"_prb%s.sgy" % (logname)
            else:
                outfcsegy = os.path.join(cmdl.outdir,wfname) +"_cls%s%d.sgy" % (logname,cmdl.classproba)
        else:
            if cmdl.outproba:
                outfpsegy = os.path.join(dirsplit,wfname) +"_prb%s.sgy" %(logname)
            else:
                outfcsegy = os.path.join(dirsplit,wfname) +"_cls%s%d.sgy" %(logname,cmdl.classproba)

        print('Copying file, please wait ........')
        start_copy = datetime.now()
        if cmdl.outproba:
            copyfile(sflist[0], outfpsegy)
        else:
            copyfile(sflist[0], outfcsegy)
        end_copy = datetime.now()
        print('Duration of copying segy file: {}'.format(end_copy - start_copy))
        ...
        print('Duration of zeroing segy file: {}'.format(end_zero - start_zero))
        ...
        for slicenum in range (sstart, send):
            ...
            print(f'wdfsa shape {wdfsa.shape}')
            classnames = wdfsa.iloc[:,-1].unique().tolist()
            print(f'***New Class names for slice {slicenum} {classnames}')
            X = wdfsa.iloc[:,4 : -1]
            y = wdfsa.iloc[:,-1]
            inshape = y.size
            print( f"size of y: {inshape}  {y.shape}")
            ...
            resampled = False
            if cmdl.balancetype == 'ros':
                ros = RandomOverSampler(random_state=0)
                X_resampled, y_resampled = ros.fit_sample(X, y)
                print(sorted(Counter(y_resampled).items()))
                resampled = True
            elif cmdl.balancetype == 'smote':
                X_resampled, y_resampled = SMOTE(k_neighbors=cmdl.nneighbors).fit_sample(X, y)
                print(sorted(Counter(y_resampled).items()))
                resampled = True
            elif cmdl.balancetype == 'adasyn':
                X_resampled, y_resampled = ADASYN(n_neighbors=cmdl.nneighbors).fit_sample(X, y)
                print(sorted(Counter(y_resampled).items()))
                resampled = True


            clf = CatBoostClassifier(iterations=cmdl.cbciterations, learning_rate=cmdl.cbclearningrate,
                        depth=cmdl.cbcdepth,loss_function='MultiClass',random_seed=42,logging_level='Silent')
            if resampled:
                clf.fit(X_resampled, y_resampled)
            else:
                clf.fit(X, y)
            # Get predictions
            yw = clf.predict(X, prediction_type='Class')
            # ypred = clf.predict(Xpred, prediction_type='RawFormulaVal')
            ywproba = clf.predict_proba(X)
            print(f'yproba {ywproba}')
            print(f'size of yproba {ywproba.shape}. classnames len {len(classnames)}')
            acscorelst.append(accuracy_score(y,yw))
            if cmdl.outproba:
                loglosslst.append(log_loss(y,ywproba))

            if slicenum == sstart:
                wellsdf = wdfsa[wdfsa.columns[:4]].copy()
                wellsdf[logname] = wdfsa[wdfsa.columns[-1]].copy()
                print(f'wellsdf shape {wellsdf.shape}  ypredproba shape {ywproba.shape}')
                if cmdl.outproba:
                    for i,c in enumerate(classnames):
                        wellsdf[c] = ywproba[:,i]
                else:
                    if cmdl.generatesamples:
                        wellsdf[lognamepred] = yw[:inshape]
                    else:
                        wellsdf[lognamepred] = yw
                ...
                if cmdl.outproba:
                    ...
                    if cmdl.generatesamples:
                        wellsdf0[lognamepred] = yw[:inshape]
                    else:
                        wellsdf0[lognamepred] = yw
            ...
            if cmdl.outproba:
                ypredproba = clf.predict_proba(Xpred)
                print(f'ypred shape: {ypred.shape} ypredproba shape {ypredproba.shape}')

            alldatas[wdfsa.columns[4]] = ypred
            # print('After Prediction........')
            # print(alldatas.head())
            slicestr = '{:.0f}'.format(zslice)
            if cmdl.outproba:
                preddf[slicestr] = ypredproba[probacol]
                for i,c in enumerate(classnames):
                    alldatas[c] = ypredproba[:,i]
            else:
                preddf[slicestr] = ypred
            ...
            if slicenum % cmdl.plotincrement == 0:
                ...
                if cmdl.outdir:
                    pdfcnfmtx = os.path.join(cmdl.outdir,fname) + "_cbccnfmtx.pdf"
                else:
                    pdfcnfmtx = os.path.join(dirsplit,fname) + "_cbccnfmtx.pdf"
                with PdfPages(pdfcnfmtx) as pdf:
                    plot_confusion_matrix(cnfmat,classnames,title='CatBoost Confusion Matrix',
                        hideplot=cmdl.hideplots)
                    pdf.savefig()
                    if not cmdl.hideplots:
                        plt.show()
                    plt.close()

                    plot_confusion_matrix(cnfmat,classnames,normalize=True,title='CatBoost Confusion Matrix Normalized',
                        hideplot=cmdl.hideplots)
                ...
                swfname = 'SWAttrib'
                if cmdl.outdir:
                    wsdf = os.path.join(cmdl.outdir,swfname)+ f"{slicedepth:.0f}" +"_cbc%s.csv" %(logname)
                else:
                    wsdf = os.path.join(dirsplit,swfname)+ f"{slicedepth:.0f}" +"_cbc%s.csv" %(logname)
        ...
        if cmdl.outproba:
            qccols = ['SLICENUM','SLICEZ','WELLSFOUND','WELLSUSED','ACCURACY','LOGLOSS']
            qcdf = pd.DataFrame({'SLICENUM':slicenumlst,'SLICEZ':slicelst,'WELLSFOUND':wnlst,'WELLSUSED':slicewnlst,
                    'ACCURACY': acscorelst ,'LOGLOSS':loglosslst})
            qcdf = qcdf[qccols].copy()
        else:
            qccols = ['SLICENUM','SLICEZ','WELLSFOUND','WELLSUSED','ACCURACY']
            qcdf = pd.DataFrame({'SLICENUM':slicenumlst,'SLICEZ':slicelst,'WELLSFOUND':wnlst,'WELLSUSED':slicewnlst,
                'ACCURACY': acscorelst })
            qcdf = qcdf[qccols].copy()
