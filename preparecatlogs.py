...
def getcommandline():
    ...
    parser.add_argument('--categorical',action='store_true',default=False,help='categorical log. default= continuous')
    parser.add_argument('--nneighbors',type=int,default=5,help='KNN n_neighbors.default= 5')
...
def main():
    ...
    if cmdl.devfilename:
        ...
        if not cmdl.categorical:
            ...
            # ax[3].imshow(cluster1, interpolation='none', aspect='auto',vmin=0,vmax=4)
            # ax[3].set_xticklabels([])
            # ax[3].set_yticklabels([])
            # ax[3].set_xticks([])
            # ax[3].set_yticks([])

            ax[0].set_xlabel(cmdl.logcolname)
            ax[0].set_ylabel('DEPTH')
            fig.suptitle(cmdl.wellname)
            fig.tight_layout()
            pdfcl = os.path.join(dirsplitlogs,fnamelogs) +"_log.pdf"
            if not cmdl.hideplot:
                plt.show()
